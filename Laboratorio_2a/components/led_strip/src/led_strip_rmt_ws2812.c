 ...
 * data
 */
void IRAM_ATTR ws2812_rmt_adapter(const void *src, rmt_item32_t *dest,
...
esp_err_t ws2812_set_pixel(led_strip_t *strip, uint32_t index, uint32_t red,
...
esp_err_t led_rgb_init(led_strip_t **strip) {
  ...
  rmt_config_t config =
      RMT_DEFAULT_CONFIG_TX(KALUGA_RGB_LED_PIN, RMT_CHANNEL_0);
  ...
  led_strip_config_t strip_config = LED_STRIP_DEFAULT_CONFIG(
      KALUGA_RGB_LED_NUMBER, (led_strip_dev_t)config.channel);
